...
(function () {
  ...
  function checkColors(opts) {
    ...
    if (hasDirectText && textColor && !isEmojiOnly) {
      ...
      if (bgs) {
        ...
        const isGray = !hasChroma(textColor, 20) && textLum > 0.05 &&
          textLum < 0.85;
        ...
        const isLargeText = fontSize >= WCAG_LARGE_TEXT_PX ||
        ...
        if (ratio < threshold) {
          ...
          const isAlphaFallbackFP = !DETECTOR_IS_BROWSER && !effectiveBg &&
            (textColor.a != null && textColor.a < 1);
          if (!isAlphaFallbackFP) {
            findings.push({
              id: "low-contrast",
              snippet: `${ratio.toFixed(1)}:1 (need ${threshold}:1) — text ${
    ...
    if (classList) {
      const classStr = typeof classList === "string"
        ? classList
      ...
      if (
        /\bbg-clip-text\b/.test(classStr) && /\bbg-gradient-to-/.test(classStr)
      ) {
        findings.push({
          id: "gradient-text",
  ...
  function checkIconTile(opts) {
    ...
    return [{
      id: "icon-tile-stack",
      snippet: `${Math.round(siblingWidth)}x${
        Math.round(siblingHeight)
  ...
  function checkItalicSerif(opts) {
    ...
    return [{
      id: "italic-serif-display",
      snippet: `italic serif ${tag} (${primary || "serif"}) at ${
        Math.round(fontSize)
      }px "${text}"`,
    }];
  }
  ...
  function isAccentColor(cssColor) {
    ...
    const rgbM = /rgba?\(\s*(\d+)\s*,?\s+|\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)/
      .exec(s.replace(/rgba?\(\s*/, "rgb(").replace(/,/g, ", "));
  ...
  function checkHeroEyebrow(opts) {
    ...
    return [{
      id: "hero-eyebrow-chip",
      snippet:
        `eyebrow chip (${style}) "${eyebrowSnippet}" above ${headingTag} "${headingTextSnippet}"`,
    }];
  }
  ...
  function checkRepeatedSectionKickers(opts) {
    ...
    return candidates.map((candidate) => ({
      id: "repeated-section-kickers",
      snippet:
        `repeated section kicker "${candidate.kickerText}" before ${candidate.headingTag} "${candidate.headingText}" (${candidates.length} on page)`,
    }));
  }
  ...
  function checkMotion(opts) {
    ...
    if (
      ...
      const props = transitionProperty.split(",").map((p) =>
        p.trim().toLowerCase()
      );
  ...
  function checkHtmlPatterns(html) {
    ...
    if (roundedSpacing.length >= 10) {
      ...
      if (dominantPct > 0.6 && unique.length <= 3) {
        ...
        findings.push({
          id: "monotonous-spacing",
          snippet:
            `~${dominant}px used ${maxCount}/${roundedSpacing.length} times (${
              Math.round(dominantPct * 100)
            }%)`,
        });
      }
    }
    ...
    if (darkBgRe.test(html) || twDarkBg.test(html)) {
      ...
      while ((shm = shadowRe.exec(html)) !== null) {
        ...
        const pxVals = [...val.matchAll(/(\d+)px|(?<![.\d])\b(0)\b(?![.\d])/g)]
          .map((p) => +(p[1] || p[2]));
        if (pxVals.length >= 3 && pxVals[2] > 4) {
          findings.push({
            id: "dark-glow",
  ...
  function resolveBackground(el, win, customPropMap) {
    ...
    while (current && current.nodeType === 1) {
      ...
      const hasGradientOrUrl = bgImage && bgImage !== "none" &&
      ...
      if (!DETECTOR_IS_BROWSER && (!bg || bg.a < 0.1)) {
        ...
        if (!bg || bg.a < 0.1) {
          ...
          const inlineBg = bgMatch ? bgMatch[1].trim() : "";
          if (
            inlineBg && !/gradient/i.test(inlineBg) &&
            !/url\s*\(/i.test(inlineBg)
          ) {
            bg = parseColorResolved(inlineBg, customPropMap) ||
  ...
  function checkElementColorsDOM(el) {
    ...
    const style = getComputedStyle(el);
    const directText = [...el.childNodes].filter((n) => n.nodeType === 3).map(
      (n) => n.textContent,
    ).join("");
  ...
  function checkElementIconTileDOM(el) {
    ...
    const sibDirectText = [...sibling.childNodes].filter((n) =>
      n.nodeType === 3
    ).map((n) => n.textContent).join("");
    const hasInlineEmojiIcon = sibling.children.length === 0 &&
  ...
  function checkElementAIPaletteDOM(el) {
    ...
    if (textColor && hasChroma(textColor, 80)) {
      const hue = getHue(textColor);
      const isAIPalette = (hue >= 160 && hue <= 200) ||
        (hue >= 260 && hue <= 310);
      if (isAIPalette) {
  ...
  function resolveFontSizePx(el, win) {
    ...
    while (cur && cur.nodeType === 1) {
      const fs =
        (win ? win.getComputedStyle(cur) : getComputedStyle(cur)).fontSize;
      chain.push(fs || "");
      cur = cur.parentElement;
    }
  ...
  function resolveLengthPx(value, fontSizePx) {
    if (
      !value || value === "normal" || value === "auto" || value === "inherit"
    ) return null;
  ...
  function checkQuality(opts) {
    ...
    if (
      rect && hasDirectText && QUALITY_TEXT_TAGS.has(tag) && rect.width > 0 &&
      textLen > lineMax
    ) {
    ...
    if (
      rect && hasDirectText && textLen > 20 && rect.width > 100 &&
      rect.height > 30
    ) {
      const borders = {
      ...
      if (borderCount >= 2 || hasBg) {
        ...
        if (vMin < vThresh) {
          findings.push({
            id: "cramped-padding",
            snippet: `${vMin}px vertical padding (need ≥${
              vThresh.toFixed(1)
          ...
          findings.push({
            id: "cramped-padding",
            snippet: `${hMin}px horizontal padding (need ≥${
              hThresh.toFixed(1)
            }px for ${fontSize}px text)`,
          });
        }
    ...
    // visual failure mode.
    {
      ...
      if (
        ...
        const outlineVisible = outlineW > 0 &&
          !isTransparent(outlineColorVal) && outlineStyleVal &&
          outlineStyleVal !== "none";
        const bgVisible = !isTransparent(style.backgroundColor);

        const anyVisible = borderVisible.top || borderVisible.right ||
          borderVisible.bottom || borderVisible.left || outlineVisible ||
          bgVisible;
        if (anyVisible) {
          ...
          for (const side of ["top", "right", "bottom", "left"]) {
            const sideBounded = borderVisible[side] || outlineVisible ||
              bgVisible;
            if (
              sideBounded && pad[side] <= PAD_THRESHOLD &&
            ...
            if (hasTextChild) {
              const cls =
                (typeof el.className === "string" && el.className.trim())
                  ...
                  : "";
              const boundaryParts = [];
              const borderSidesVisible = ["top", "right", "bottom", "left"]
              ...
              const sidesLabel = flushSides.length === 4
                ? "all sides"
                : flushSides.join("/");
              const ident = cls
                ? `<${tag.toLowerCase()}> "${cls}"`
              ...
              findings.push({
                id: "cramped-padding",
                snippet: `${ident}: children flush against ${
                  boundaryParts.join("+")
    ...
    if (
      rect && hasDirectText && textLen > 40 &&
      ["P", "LI"].includes(tag.toUpperCase()) && viewportWidth > 0
    ) {
      const inNavHeader = el.closest &&
        (el.closest("nav") || el.closest("header"));
      const hasOwnBg = style.backgroundColor &&
        style.backgroundColor !== "rgba(0, 0, 0, 0)" &&
      ...
      if (
        !inNavHeader && !hasOwnBg && !isPositioned && widthRatio > 0.5 &&
        (leftClose || rightClose)
      ) {
        const which = leftClose && rightClose
          ? `left ${Math.round(rect.left)}px / right ${
        ...
        findings.push({
          id: "body-text-viewport-edge",
          snippet:
            `<${tag.toLowerCase()}> with ${textLen}-char body bleeds to viewport edge (${which})`,
        });
      }
    }
  ...
  function checkElementQualityDOM(el) {
    ...
    const rect = el.getBoundingClientRect();
    const lineMax = (typeof window !== "undefined" &&
  ...
  function checkPageQualityFromDoc(doc) {
    ...
    for (const h of headings) {
      ...
      const text = (h.textContent || "").trim().replace(/\s+/g, " ").slice(
        0,
        60,
      );
      if (prevLevel > 0 && level > prevLevel + 1) {
        findings.push({
          id: "skipped-heading",
          snippet:
            `<h${prevLevel}> "${prevText}" followed by <h${level}> "${text}" (missing h${
              prevLevel + 1
            })`,
        });
      }
  ...
  function checkElementBorders(tag, style, overrides, resolvedRadius) {
    ...
    for (const s of sides) {
      ...
      if (widths[s] === 0 && overrides && overrides[s]) {
        ...
        colors[s] && colors[s].startsWith("var(") && overrides && overrides[s]
      ) {
  ...
  function checkElementIconTile(el, tag, window) {
    ...
    if (iconChild) {
      ...
      iconWidth = parseFloat(iconStyle.width) ||
        parseFloat(iconChild.getAttribute("width")) || 0;
    }
    ...
    if (sizes.size >= 3) {
      ...
      if (ratio < 2.0) {
        findings.push({
          ...
          detail: `Sizes: ${sorted.map((s) => s + "px").join(", ")} (ratio ${
            ratio.toFixed(1)
          }:1)`,
        });
      }
  ...
  function checkPageTypography(doc, win) {
    ...
    while ((m = gfRe.exec(html)) !== null) {
      const families = m[1].split("|").map((f) =>
    ...
    if (sizes.size >= 3) {
      ...
      if (ratio < 2.0) {
        findings.push({
          id: "flat-type-hierarchy",
          snippet: `Sizes: ${sorted.map((s) => s + "px").join(", ")} (ratio ${
            ratio.toFixed(1)
          }:1)`,
        });
      }
  ...
  function checkOversizedH1({ tag, fontSize, headingText }) {
    ...
    if (fontSize >= OVERSIZED_H1_FONT_PX && textLen >= OVERSIZED_H1_MIN_CHARS) {
      return [{
        id: "oversized-h1",
        snippet: `${Math.round(fontSize)}px h1, ${textLen} chars "${
          headingText.slice(0, 60)
        }"`,
      }];
    }
    return [];
  ...
  function checkElementTextOverflowDOM(el) {
    ...
    if (el.clientWidth > 0 && delta >= 16) {
      return [{
        id: "text-overflow",
        snippet: `${classSelector(el)} overflows its box by ${
          Math.round(delta)
        }px`,
      }];
    }
  ...
  `;
          ...
          if (!overlay._revealed) {
            ...
            if (firstScanDone) {
              ...
              overlay.style.animationDelay = `${
                Math.min((overlay._staggerIndex || 0) * 60, 600)
              }ms`;
            }
    ...
    function detachOverlay(overlay) {
      ...
      if (
        overlay._targetEl && overlay._targetEl._impeccableOverlay === overlay
      ) {
    ...
    document.addEventListener("transitionend", (e) => {
      ...
      for (const o of overlays) {
        if (
          !o._targetEl || o.classList.contains("impeccable-banner") ||
      ...
      if (parent) {
        try {
          ...
          if (matching.length > 1) {
            const sameType = [...parent.children].filter((c) =>
    ...
    function generateSelector(el) {
      ...
      while (
        ...
        current !== document.documentElement && depth < MAX_DEPTH
      ) {
    ...
    function collectVisualContrastReasons(el, style) {
      ...
      while (current && current.nodeType === 1) {
        ...
        if (
          currentStyle.mixBlendMode && currentStyle.mixBlendMode !== "normal"
        ) reasons.add("blend mode");
        if (currentStyle.filter && currentStyle.filter !== "none") {
          reasons.add("filter");
        }
        if (
          currentStyle.backdropFilter && currentStyle.backdropFilter !== "none"
      ...
      if (sampleRect && document.elementsFromPoint) {
        ...
        for (const [x, y] of points) {
          ...
          const selfIndex = stack.findIndex((node) =>
          ...
          for (const node of stack.slice(selfIndex + 1)) {
            ...
            if (
              nodeTag === "img" || nodeTag === "picture" ||
              nodeTag === "video" || nodeTag === "canvas" || nodeTag === "svg"
            ) {
    ...
    function collectVisualContrastCandidates(options = {}) {
      ...
      for (const el of document.querySelectorAll("*")) {
        ...
        candidates.push({
          ...
          clip,
          textColor,
          preferRenderedForeground: !textColor || textColor.a < 0.99 ||
            reasons.some((reason) =>
    ...
    function resolvePaintedImageRect(
      ...
      if (size === "cover" || size === "contain") {
        ...
        if (heightToken === "auto") {
          ...
          paintedHeight = containerRect.height *
      ...
      const promise = new Promise((resolve) => {
        ...
        try {
          ...
          if (
            ...
            absolute.protocol !== "data:" && absolute.protocol !== "blob:"
          ) {
    ...
    async function sampleVisualBackgroundAtPoint(
      ...
      for (const node of nodes) {
        ...
        if (sample.status === "sampled") {
          if (
            !sample.color || sample.color.a == null || sample.color.a >= 0.95
          ) return sample;
    ...
    async function analyzeVisualContrastCandidate(candidate) {
      ...
      const detail = `browser contrast ${measuredRatio.toFixed(1)}:1 median ${
        medianRatio.toFixed(1)
      }:1 (need ${candidate.threshold}:1) via ${method}${textLabel}`;
      return {
        ...candidate,
        status,
    ...
    async function analyzeVisualContrast(options = {}) {
      ...
      for (const candidate of candidates) {
        ...
        if (
          shouldScrollOffscreen && result.status === "unresolved" &&
    ...
    function serializeFindings(allFindings) {
      return allFindings.map(({ el, findings }) => ({
        ...
        rect: (el !== document.body && el !== document.documentElement &&
    ...
    function collectBrowserFindings() {
      ...
      for (
        const node of docClone.querySelectorAll('[id^="impeccable-live-"]')
      ) {
        node.remove();
      }
    ...
    function rememberVisualContrastAnalysis(result) {
      ...
      const idx = lastVisualContrastAnalyses.findIndex((item) =>
        item.selector === result.selector
      );
    ...
    function addVisualContrastResult(groupMap, result, options = {}) {
      ...
      if (
        options.decorate && el !== document.body &&
