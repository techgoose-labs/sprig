...
function checkColors(opts) {
  ...
  if (hasDirectText && textColor && !isEmojiOnly) {
    ...
    const bgs = effectiveBg
      ? [effectiveBg]
      : (effectiveBgStops && effectiveBgStops.length ? effectiveBgStops : null);
    if (bgs) {
  ...
  while (current && current.nodeType === 1) {
    ...
    if (!DETECTOR_IS_BROWSER && (!bg || bg.a < 0.1)) {
      ...
      if (!bg || bg.a < 0.1) {
        ...
        const inlineBg = bgMatch ? bgMatch[1].trim() : "";
        if (
          inlineBg && !/gradient/i.test(inlineBg) && !/url\s*\(/i.test(inlineBg)
        ) {
...
function checkElementIconTileDOM(el) {
  ...
  const sibDirectText = [...sibling.childNodes].filter((n) => n.nodeType === 3)
...
function buildCustomPropMap(document) {
  ...
  for (const sheet of sheets) {
    ...
    for (const rule of rules) {
      ...
      if (
        rule.type === 4 /* MEDIA_RULE */ || rule.type === 12 /* SUPPORTS_RULE */
      ) {
        try {
...
function checkQuality(opts) {
  ...
  // visual failure mode.
  {
    ...
    if (
      ...
      const outlineVisible = outlineW > 0 && !isTransparent(outlineColorVal) &&
        outlineStyleVal && outlineStyleVal !== "none";
      ...
      if (anyVisible) {
        ...
        for (const side of ["top", "right", "bottom", "left"]) {
          const sideBounded = borderVisible[side] || outlineVisible ||
            bgVisible;
          if (
            sideBounded && pad[side] <= PAD_THRESHOLD && !childrenInsulate[side]
          ) {
