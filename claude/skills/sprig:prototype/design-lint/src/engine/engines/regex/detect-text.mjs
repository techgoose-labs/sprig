...
const REGEX_MATCHERS = [
  ...
  // --- Layout property transition ---
  {
    ...
    test: (m) => {
      ...
      return /\b(?:(?:max|min)-)?(?:width|height)\b|\bpadding\b|\bmargin\b/
        .test(val);
    },
    fmt: (m) => {
  ...
  (content, filePath) => {
    ...
    return [
      finding(
        "flat-type-hierarchy",
        filePath,
        `Sizes: ${sorted.map((s) => s + "px").join(", ")} (ratio ${
          ratio.toFixed(1)
        }:1)`,
        line,
      ),
  ...
  (content, filePath) => {
    ...
    return [
      finding(
        "monotonous-spacing",
        filePath,
        `~${dominant}px used ${maxCount}/${rounded.length} times (${
          Math.round(pct * 100)
        }%)`,
      ),
    ];
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        while ((m = matcher.regex.exec(line)) !== null) {
          ...
          const context = blockContext
            ? lines.slice(Math.max(0, i - 3), Math.min(lines.length, i + 4))
              .join(" ")
            : line;
