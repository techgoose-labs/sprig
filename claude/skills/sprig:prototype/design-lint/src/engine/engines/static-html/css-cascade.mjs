...
function normalizeStaticCssValue(
  ...
  const isModernBorderColor = /^border[A-Z][a-z]+Color$/.test(prop) &&
  ...
  if (prop === "letterSpacing") {
    const base = parseFloat(currentStyle?.fontSize || parentStyle?.fontSize) ||
      16;
    const px = resolveLengthPx(resolved, base);
    if (px != null) resolved = `${px}px`;
  }
...
function expandStaticDeclaration(prop, value) {
  ...
  if (
    STATIC_DEFAULT_STYLE[mapped] != null || STATIC_INHERITED_PROPS.has(mapped)
  ) {
    return [[mapped, v]];
  }
  return [];
...
function applyStaticDeclaration(specified, node, prop, value, meta) {
  ...
  for (
    const [expandedProp, expandedValue] of expandStaticDeclaration(prop, value)
  ) {
...
class StaticElement {
  ...
  get children() {
    return (this.node.children || []).filter((child) => child.type === "tag")
  ...
  querySelectorAll(selector) {
    try {
      return this._doc.selectAll(selector, this.node.children || []).map(
        (node) => this._doc.wrap(node),
      );
    } catch {
      return [];
    }
...
function collectStaticCssText(root, fileDir, profile, filePath, modules) {
  ...
  for (const link of links) {
    ...
    if (
      !/\bstylesheet\b/i.test(rel) || !href || /^(https?:)?\/\//i.test(href)
    ) continue;
