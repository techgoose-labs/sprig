...
async function captureVisualContrastCandidate(page, candidate, viewport) {
  ...
  const token = `impeccable-contrast-${Date.now()}-${
  ...
  if (
    !metrics || !Number.isFinite(metrics.p10Ratio) || metrics.glyphPixels < 8
  ) return null;
  ...
  const reasonLabel = (candidate.reasons || []).slice(0, 3).join(", ") ||
    "visual background";
  return {
    id: "low-contrast",
    snippet: `pixel contrast ${measuredRatio.toFixed(1)}:1 median ${
      metrics.medianRatio.toFixed(1)
    }:1 (need ${candidate.threshold}:1) on ${reasonLabel}${textLabel}`,
  };
}
