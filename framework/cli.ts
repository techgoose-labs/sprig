...
//
// The 1.x CLI wrote its own marker. A serve.ts carrying it is by definition
// generated — refusing to regenerate it stranded every 1.x app at "already
// exists and was NOT generated" on its first 2.x build, one step into the
// migration. So it counts as generated too, for regeneration and for clean.
const LEGACY_SERVE_MARKER = "GENERATED by `sprig build --rune`";
function isGeneratedServe(src: string): boolean {
  return src.includes(BEDROCK_SERVE_MARKER) || src.includes(LEGACY_SERVE_MARKER);
}
/** Where the 1.x → 2.x hand-migration steps live (ships in the docs of every release). */
const MIGRATION_GUIDE =
  "https://github.com/techgoose-labs/sprig/blob/main/docs/sprig/migrating-1x-to-2x.md";
...
async function migrateLegacyRuntime(appDir: string): Promise<void> {
    ...
    for (const name of ["deno.json", "deno.jsonc"]) {
      ...
      if (tasks) {
        for (const [tk, tv] of Object.entries(tasks)) {
          if (typeof tv === "string" && hasLegacyRuntimeName(tv)) {
  ...
  // 2) SOURCE IMPORTS — rewrite the specifiers across the app tree, with the ONE rewrite the
  //    config pass uses (pin-stamp.ts), so the two can't disagree on which names are legacy:
  //    they did, and a 1.x app came out of `sprig build` with its deno.json migrated and every
  //    ui/src import still on `@mrg-keystone/sprig` — a bundle that fails on the first import.
  ...
  if (configs || files) {
    console.log(
      `sprig: migrated legacy sprig imports → @techgoose-labs/sprig (${configs} config(s), ${files} source file(s)).\n` +
        `  What a 1.x app still changes by hand (serve.ts, wire paths, /api/docs):\n` +
        `  ${MIGRATION_GUIDE}`,
    );
  }
}
    ...
    if (!isGeneratedServe(cur)) {
      console.error(
        `sprig build --rune: ${servePath} already exists and was NOT generated.\n` +
          ...
          `  Porting a hand-written 1.x serve.ts (api.compose → Bedrock)? ${MIGRATION_GUIDE}`,
      );
      Deno.exit(1);
    }
    if (cur.includes(LEGACY_SERVE_MARKER)) {
      console.log(
        `sprig build: regenerating the 1.x serve.ts as a Bedrock composition root — see ${MIGRATION_GUIDE}`,
      );
    }
  }
...
async function init(dir = "."): Promise<void> {
  ...
  const files: Record<string, string> = {
    ...
    // `minimumDependencyAge`: Deno's 24h supply-chain cooldown, off — the app pins
    // sprig to the EXACT CLI version (stamp), which is usually hours old right after a
    // `sprig update`, and the bundle then fails on "blocked by the minimum dependency
    // age policy". (The per-package `exclude` form is rejected by Deno 2.9.)
    "deno.json": `{
  "tasks": {
    "dev": "sprig dev",
    "build": "sprig build"
