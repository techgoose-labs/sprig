...
Deno.test("softNavResponseOk: a followed redirect is NOT committable (guard 302 → full nav); a plain page is", () => {
  assertEquals(
    softNavResponseOk(html("<p>ok</p>")),
    true,
    "2xx text/html, not redirected → soft-swap",
  );
  assertEquals(
    softNavResponseOk(redirectedResponse("<p>login</p>")),
    false,
    "redirected → full navigation",
  );
  assertEquals(
    softNavResponseOk(
      new Response("nope", {
...
Deno.test("runSoftNav on a guard redirect: full navigation to the ORIGINAL destination, outlet untouched", async () => {
  ...
  const deps: SoftNavDeps = {
    ...
    fetch: () =>
      Promise.resolve(
        redirectedResponse("<sprig-outlet><p>login</p></sprig-outlet>"),
      ),
    parse: () => {
      calls.push("parse");
  ...
  assertEquals(
    calls,
    ["assign:https://app.test/ui/admin"],
    "redirect → assign only; no parse/swap/bootstrap",
  );
});
  ...
  try {
    const deps: SoftNavDeps = {
      fetch: () =>
        Promise.resolve(
          html(
            `<html><body><sprig-outlet><p>new</p></sprig-outlet></body></html>`,
          ),
        ),
      parse: (h) =>
        new DOMParser().parseFromString(h, "text/html") as unknown as Document,
    ...
    assert(
      !calls.some((c) => c.startsWith("assign:")),
      "committable page must NOT full-navigate",
    );
    assertEquals(
      calls,
      ["teardown", "bootstrap"],
      "the outlet was torn down and re-armed",
    );
    assertEquals(
      live.querySelector("sprig-outlet")!.innerHTML,
      "<p>new</p>",
      "outlet content swapped in place",
    );
  } finally {
    // deno-lint-ignore no-explicit-any
