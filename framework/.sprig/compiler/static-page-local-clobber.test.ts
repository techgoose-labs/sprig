...
Deno.test("BUG S: same-basename static card resolves per-page on the client (page-local vs global), no silent clobber", async () => {
  ...
  const globalCard = serialize(
    await parseTemplate(`<p class="card">GLOBAL-CARD</p>`),
  );
  const homeCard = serialize(
    await parseTemplate(`<p class="card">HOME-CARD</p>`),
  );
  registerComponent("card", { template: globalCard, scope: "sc-global" });
  registerPageComponent("home", "card", {
    template: homeCard,
    scope: "sc-home",
  });
  ...
  const islandTpl = serialize(
    await parseTemplate(`<div class="wrap"><card></card></div>`),
  );
  ...
  try {
    ...
    assert(
      home.innerHTML.includes("HOME-CARD"),
      "home island must render its PAGE-LOCAL card text",
    );
    assert(
      home.innerHTML.includes("sc-home"),
      "home island must render the PAGE-LOCAL card scope marker",
    );
    assert(
      !home.innerHTML.includes("GLOBAL-CARD"),
      "home island must NOT render the global card (clobber)",
    );

    // ABOUT page: no page-local card → the GLOBAL card (GLOBAL-CARD + sc-global).
    assert(
      about.innerHTML.includes("GLOBAL-CARD"),
      "about island must render the GLOBAL card text",
    );
    assert(
      about.innerHTML.includes("sc-global"),
      "about island must render the GLOBAL card scope marker",
    );
    assert(
      !about.innerHTML.includes("HOME-CARD"),
      "about island must NOT render home's page-local card",
    );
  } finally {
    unmockDoc();
  }
...
Deno.test("BUG S: an island host WITHOUT data-page falls back to __sprig_config.page (bootstrapIslands)", async () => {
  ...
  const islandTpl = serialize(
    await parseTemplate(`<div class="wrap2"><card></card></div>`),
  );
  const entry: IslandEntry = {
    setup: () => ({}),
    template: islandTpl,
  ...
  try {
    ...
    assert(
      host.innerHTML.includes("HOME-CARD"),
      "island with no data-page resolves <card> via cfg.page=home → page-local",
    );
    assert(
      !host.innerHTML.includes("GLOBAL-CARD"),
      "must not clobber to the global card",
    );
  } finally {
    unmockDoc();
  }
...
Deno.test("BUG S: soft-nav threads the NEW page into cfg so swapped-in islands resolve per the new page", async () => {
  ...
  const islandTpl = serialize(
    await parseTemplate(`<div class="wrap3"><card></card></div>`),
  );
  const entry: IslandEntry = {
    setup: () => ({}),
    template: islandTpl,
  ...
  const nextHtml = `<html><body><sprig-outlet>` +
  ...
  try {
    ...
    assert(
      ...
      "pageFromConfig reads page=home",
    );
    ...
    assert(
      host.innerHTML.includes("HOME-CARD"),
      "after soft-nav to home, the island resolves <card> to the page-local card",
    );
    assert(
      !host.innerHTML.includes("GLOBAL-CARD"),
      "after soft-nav to home it must NOT render the global card",
    );
  } finally {
    unmockDoc();
  }
