...
Deno.test("the Dooks contract: onServerInit's value crosses to onBrowserInit", async () => {
  ...
  assertEquals(
    html,
    "<p>1</p>",
    "render must read the post-onServerInit value",
  );
  ...
  assertEquals(
    browserSaw,
    1,
    "onBrowserInit must see the server-produced 1, not the field-init 0",
  );
});
...
Deno.test("signal VALUES ride along in the snapshot and re-seed reactive state", async () => {
  ...
  assertEquals(
    snapshot,
    { count: 5 },
    "the signal's value is snapshotted, not the signal object",
  );
  ...
  assertEquals(
    inst.count(),
    5,
    "the browser signal is re-seeded to 5 and stays reactive",
  );
  inst.count.set(6);
...
Deno.test("non-serializable fields are dropped, methods come from the class", async () => {
  ...
  assertEquals(
    snapshot,
    { data: 9 },
    "the closure is silently excluded from the snapshot",
  );
  ...
  assertEquals(
    inst.cb(),
    99,
    "the closure is the fresh one from `new WithFn()`, not transferred",
  );
});
...
Deno.test("onBrowserDestroy cleans up (no work after teardown)", async () => {
  ...
  assertEquals(
    inst.ticks,
    atDestroy,
    "no ticks after onBrowserDestroy — the interval is gone",
  );
});
